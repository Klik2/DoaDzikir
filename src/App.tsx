    ...
    <div id="app-root" className={`min-h-screen flex flex-col transition-all duration-500 ${themeClass} ${contrastClass} ${fontClass}`}>
                        ...
                        return (
                          ...
                          >
                            <div className="flex flex-col gap-1">
                               <span className="font-black text-sm uppercase tracking-tight">{d.title}</span>
      ...
      <footer className={`sticky bottom-0 p-4 md:p-6 flex flex-col gap-4 z-50 border-t ${isA11yMode ? 'bg-black border-yellow-300' : isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-stone-200'}`}>
        <div className="flex gap-4 md:gap-8 w-full font-black">
          <button 
            onClick={handlePrev}
            className={`flex-1 p-6 md:p-8 rounded-[35px] text-2xl md:text-3xl transition-all active:scale-95 border-b-8 focus:ring-8 focus:ring-blue-400 outline-none ${isA11yMode ? 'bg-black border-yellow-300 text-yellow-300 shadow-[10px_10px_0px_#fde047]' : isDarkMode ? 'bg-slate-800 border-slate-700 text-white shadow-xl' : 'bg-slate-100 border-slate-200 text-slate-800 shadow-lg'}`}
          >
            <ChevronLeft size={isA11yMode ? 64 : 32} className="inline mr-2" /> <span className="hidden md:inline">{t.prev}</span>
          </button>
          
          <button 
            onClick={handleNext}
            className={`flex-1 p-6 md:p-8 rounded-[35px] text-2xl md:text-3xl transition-all active:scale-95 border-b-8 focus:ring-8 focus:ring-blue-400 outline-none ${isA11yMode ? 'bg-yellow-300 border-yellow-400 text-black shadow-[10px_10px_0px_#ffffff]' : isDarkMode ? 'bg-blue-600 border-blue-700 text-white shadow-xl' : 'bg-slate-900 border-slate-800 text-white shadow-2xl'}`}
          >
            <span className="hidden md:inline">{t.next}</span> <ChevronRight size={isA11yMode ? 64 : 32} className="inline ml-2" />
          </button>
        </div>
